...
public class ExcelImportServer extends ImportBaseService {
    ...
    private <T> List<T> importExcel(Collection<T> result, Sheet sheet, Class<?> pojoClass,
        ...
        while (rows.hasNext()
            ...
            if (params.getKeyIndex() != null
                ...
                try {
                    int firstIndexKey =row.getFirstCellNum();
                    //获取标签开始的列序号值
                    if(!titlemap.isEmpty()){
                        Map.Entry<Integer,String> entry = titlemap.entrySet().iterator().next();
                        firstIndexKey = entry.getKey();
                    }
                    //for (int i = firstIndexKey, le = titlemap.size()+firstIndexKey; i < le; i++) {
                    for (Integer i:titlemap.keySet()) {
                        Cell cell = row.getCell(i);
                        String titleString = (String) titlemap.get(i);
    ...
    private Map<Integer, String> getTitleMap(Iterator<Row> rows, ImportParams params,
        ...
        Row row = null;
        for (int j = params.getStartRows(); j < params.getHeadRows(); j++) {
            row = rows.next();
            if (row == null) {
                continue;
            }
    ...
    public ExcelImportResult importExcelByIs(Workbook book, Class<?> pojoClass,
                                             ImportParams params) throws Exception {
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("Excel import start ,class is {}", pojoClass);
        }
